module.exports = function (app, request, ports, hostname) {
    app.post('/endpoints/login', function (req, res) {
        var options = {
            method: 'POST',
            uri: `${req.protocol}://${hostname}:${ports.authentication}${process.env.LOGIN_ENDPOINT}`,
            body: req.body,
            json: true
        };
    ...
    app.post('/endpoints/signup', function (req, res) {
        ...
        var options = {
            method: 'POST',
            uri: `${req.protocol}://${hostname}:${ports.authentication}${process.env.SIGNUP_ENDPOINT}`,
            body: req.body,
            json: true
        };

        console.log('erneseeeee: ' + JSON.stringify(options));
        request.post(options, function (error, response, body) {
